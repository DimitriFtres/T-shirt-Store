<?php
    ...
    //TRAITER LES INFORMATIONS DE POUR CREER UN NOUVEAU ENREGISTREMENT
        $nouveau = $bdd->prepare("INSERT INTO teeshirts (Nom, Quantite_stock, Auteur, Image, Categorie, Prix)
                                VAlUES (?,?,?,?,?,?,?)");
        ...
        if((is_numeric($_POST["quantite"])) AND (rendre_le_prix_float($_POST["prix"]) !== 0)){
            if($retour = imagetest($_FILES["image"])){
                if($nouveau ->execute(array($_POST["nom"], $_POST["quantite"], $_POST["auteur"], $retour[1], $_POST["categorie"], $_POST["prix"]))){
